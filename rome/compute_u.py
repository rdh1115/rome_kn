...
def compute_u(
    ...
    elif hparams.fact_token == 'kn':
        kn = KnowledgeNeurons(model, tok, model_type='gpt2', device='cuda')
        text = [request["prompt"].format(request["subject"])]
        ground_truth = request["target_true"]["str"]
        refined_neuron_idx = kn.get_refined_neurons(
            text,
            ground_truth)[0][1]
        cur_repr = repr_tools.get_reprs_at_idxs(
            contexts=[
            ...
            idxs=[[refined_neuron_idx] for _ in range(len(context_templates))],
            **word_repr_args,
        ).mean(0)
    else:
